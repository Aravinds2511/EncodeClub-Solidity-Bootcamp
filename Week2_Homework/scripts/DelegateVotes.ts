...
const delegatorPrivateKey = process.env.O_PRIVATE_KEY || "";
...
async function main() {
  ...
  const AddressToDelegate = parameters[1] as `0x${string}`;
  if (!AddressToDelegate) throw new Error("AddressToDelegate not provided");
  if (!/^0x[a-fA-F0-9]{40}$/.test(AddressToDelegate))
  ...
  console.log("Address to delegateVote: ");
  const address = AddressToDelegate;
