...
describe("Basic tests for understanding ERC20", async () => {
  it("triggers the Transfer event with the address of the sender when sending transactions", async () => {
    ...
    const hash1 = await tokenContract.write.transfer([account1.account.address, 2n]);
    ...
    expect(withdrawalEvents[0].args.value).to.equal(2n);
  });
});

// last excecution of the function is only taken into consideration.