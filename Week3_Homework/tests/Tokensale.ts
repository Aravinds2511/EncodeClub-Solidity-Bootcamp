...
describe("NFT Shop", async () => {
  ...
  describe("When a user buys an ERC20 from the Token contract", async () => {  
    ...
    it("gives the correct amount of tokens", async () => {
      const {publicClient, acc1, tokenContract, tokenSaleContract} = await loadFixture(deployFunction);
      const balanceBefore = await tokenContract.read.balanceOf([acc1.account.address])
      const buyTokensTx = await tokenSaleContract.write.buyTokens({value: TEST_BUY_TOKENS_VALUE, account: acc1.account});
  ...
  describe("When a user burns an ERC20 at the Shop contract", async () => {
    ...
    it("burns the correct amount of tokens", async () => {
      ...
      const burnTokenTx = await tokenSaleContract.write.returnTokens([TEST_BUY_TOKENS_VALUE / 2n], {account: acc1.account});
      (await publicClient).getTransactionReceipt({ hash: burnTokenTx }); 
      ...
      expect(diff).to.eq(TEST_BUY_TOKENS_VALUE / 2n / TEST_RATIO);
    });
  })
